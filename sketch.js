...
let memories; // memory girl scene
...
// Track which music is currently playing to avoid overlaps
...
function preload() {
  ...
  ringtossMemory = loadSound("assets/ringtossmemory.mp3",
    () => console.log("Action click sound loaded successfully"),
    () => console.log("Error loading ringtoss sound")
  );
}
...
function setup() {
  ...
  playButton.mousePressed(() => {
    ...
    charIndex = 0;
    lastUpdate = millis();
    // Auto-start prologue music
    playSceneMusic("prologue");
  });
  ...
  exitButton.mousePressed(() => {
    ...
    goToMap();  // Changed to go back to map instead of scene2.0
    exitButton.hide();
    // Reset bloody scene flag when leaving
    showBloodyScene = false;
    // Auto-start map music (which stops music)
    playSceneMusic("map");
...
proceedButton.mousePressed(() => {
  ...
  prologueMusicStarted = false;  // added
  showBloodyScene = false;
  playSceneMusic("map");
});
  ...
  // Setup HTML controls (simplified - remove music button functionality)
  setupHTMLControls();
  ...
  // Auto-start homepage music
  playSceneMusic("start");
}
...
function setupHTMLControls() {
  ...
  // Remove the music toggle button functionality - music now plays automatically
  const toggle = document.getElementById('toggle-sound');
  if (toggle) {
    // You can either remove this button from your HTML or leave it disabled
    toggle.style.display = 'none'; // Hide the button
  }
}
...
function mouseReleased() {
  ...
  if (bunnyDragging) {
    if (
      ...
      // ❌ Not dropped in inventory → snap back
      bunnyDragging = false;
      bunnyX = width / 2;
      bunnyY = height * 0.65;
    }

    return; // ⬅ stop here so map clicks never run
  }

  // --- Map click logic ---
  if (scene === "map" && mapClickable) {
    for (let area of mapClickAreas) {
      if (
        mouseX > area.minX && mouseX < area.maxX &&
        mouseY > area.minY && mouseY < area.maxY
      ) {
        playActionClick();
        scene = area.scene;
...
function windowResized() {
 let w, h;
  ...
  centerButtonOnCanvas(exitButton, 40);
  centerButtonOnCanvas(proceedButton, 40);
  centerButtonOnCanvas(memoryExitButton, 40); // Add this line
